...
class SaleOrder(models.Model):
    ...
    #     logger.debug("### OVERRIDE METHOD CREATE SALE ORDER ###")
    #     logger.debug(vals)
    #     return super(SaleOrder, self).create(vals)

    # def write(self, vals):
    #     logger.debug("### OVERRIDE METHOD WRITE SALE ORDER ###")
    #     logger.debug(vals)
    ...
    def mdk_create(self, order_data, **kwargs):
        ...
        if order_data.get('channel_order_reference'):
            order_ref = order_data.get('channel_order_reference')
            # Search by order ref
            order_exists = self.search(['|', ('name', '=', order_ref), ('channel_order_reference', '=', order_ref)], limit=1)
            if not force_creation and order_exists:
                logger.debug("### ORDER EXISTS {} ###".format(order_ref))
    ...
    def deliver_order(self, order_id=None, order=None, **kwargs):
        ...
        if config.dropship_enabled and config.dropship_picking_type:
            ...
            for picking in order.picking_ids:
                ...
                if picking_type.id == config.dropship_picking_type.id:
                    post_message = 'Dropship Operation'
        ...
        for picking in order.picking_ids:
            ...
            if current_delivery.state == "confirmed":
                logger.debug("Assign confirmed picking")
                try:
            ...
            if current_delivery.state == "assigned":
                logger.debug("Complete assigned picking")
                try:
                    for line in current_delivery.move_lines:
                        line.write({
